...
class EdgegridAuthEnv(AuthBase):
    ...
    def __init__(self, host: str, client_token: str, client_secret: str, access_token: str):
...
class ApiContext:
    ...
    def build_url(self, path: str, params: Optional[Dict[str, Any]] = None) -> Tuple[str, Dict[str, Any]]:
...
def papi_list_property_versions(api: ApiContext, property_id: str) -> List[Dict[str, Any]]:
    data = api.get(f"papi/v1/properties/{property_id}/versions")
    return data.get("versions", {}).get("items", [])


def papi_get_hostnames(api: ApiContext, property_id: str, version: int, contract_id: str, group_id: str) -> List[Dict[str, Any]]:
    data = api.get(
...
def papi_get_rules(api: ApiContext, property_id: str, version: int, contract_id: str, group_id: str) -> Dict[str, Any]:
    data = api.get(
...
def appsec_list_policies(api: ApiContext, config_id: int, version: int) -> List[Dict[str, Any]]:
    try:
        data = api.get(f"appsec/v1/configs/{config_id}/versions/{version}/security-policies")
...
def parse_behaviors(rules_root: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for node in flatten_rules(rules_root):
        for behavior in node.get("behaviors", []) or []:
            ...
            elif name in ("modifyOutgoingResponseHeader", "modifyOutgoingRequestHeader"):
                parsed["headers"].append({"name": name, **options})
            elif name == "hsts" or name == "httpStrictTransportSecurity" or name == "setHsts" or name == "setResponseHeader" and options.get("headerName", "").lower() == "strict-transport-security":
...
def gtm_get_property(api: ApiContext, domain: str, property_name: str) -> Dict[str, Any]:
    try:
    ...
    for path in ("cloud-wrapper/v1/containers", "cloud-wrapper/v1/configurations"):
        try:
            ...
            if isinstance(data, dict):
                items = data.get("items") or data.get("containers") or data.get("configurations") or []
                if items:
                    return items
...
def generate_checklist(apex: str, host_records: List[HostnameRecord], parsed_by_property: Dict[str, Dict[str, Any]], cps_by_sni: Dict[str, List[str]]) -> str:
    lines: List[str] = []
    ...
    for hr in sorted(host_records, key=lambda r: r.hostname):
        lines.append(f"- {hr.hostname} (property {hr.property_name} v{hr.property_version})")
    lines.append("")
...
def build_api_context(section: Optional[str], account_switch_key: Optional[str]) -> ApiContext:
    ...
    if not all([host, client_token, client_secret, access_token]):
        print("Missing EdgeGrid env vars. Please set AKAMAI_HOST, AKAMAI_CLIENT_TOKEN, AKAMAI_CLIENT_SECRET, AKAMAI_ACCESS_TOKEN.", file=sys.stderr)
        sys.exit(2)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Akamai usage reporter → Cloudflare checklist")
    parser.add_argument("--out-dir", default="./out", help="Output directory for reports")
    parser.add_argument("--include-rules", action="store_true", help="Fetch and parse rules for caching/redirects/headers/HSTS")
    parser.add_argument("--edgerc-section", default=None, help="Unused for now; placeholder if edgerc support is added")
    parser.add_argument("--account-switch-key", default=env_or_none("AKAMAI_ACCOUNT_SWITCH_KEY"))
    parser.add_argument("--include-products", action="store_true", help="Enumerate Edge DNS zones, GTM, EdgeWorkers, Cloudlets, Cloud Wrapper")
    ...
    for prop in properties:
        ...
        latest_version = prop.get("latestVersion") or prop.get("productionVersion") or prop.get("stagingVersion")
        if not property_id or not latest_version:
            continue
        contract_id = prop.get("contractId")
        group_id = prop.get("groupId")
        try:
            hostnames = papi_get_hostnames(api, property_id, int(latest_version), contract_id, group_id)
        except Exception:
            hostnames = []
        for hn in hostnames:
        ...
        if args.include_rules:
            try:
                rules = papi_get_rules(api, property_id, int(latest_version), contract_id, group_id)
                parsed_by_property[f"{property_id}:{latest_version}"] = parse_behaviors(rules)
            except Exception:
                pass
    ...
    for enr in cps:
        sans = enr.get("csr", {}).get("sans", []) or enr.get("networkConfiguration", {}).get("sanEntries", []) or []
        common_name = enr.get("csr", {}).get("cn") or enr.get("certificate", {}).get("cn")
        ...
        cps_rows.append([
            enr.get("id"), common_name or "", ";".join(sans), enr.get("status"), enr.get("deploymentSchedule", {}).get("network", "")
        ])
    ...
    for cfg in appsec_configs:
        ...
        policies = appsec_list_policies(api, int(config_id), int(latest)) if (config_id and latest) else []
    ...
    if args.include_products:
        ...
        products_rows["edgedns_zones.csv"] = [[z.get("zone" ) or z.get("name"), z.get("type") or z.get("contractId"), z.get("status") or ""] for z in zones]

        gtm_domains = gtm_list_domains(api)
        products_rows["gtm_domains.csv"] = [[d] for d in gtm_domains]

        ews = edgeworkers_list(api)
        products_rows["edgeworkers.csv"] = [[ew.get("edgeWorkerId"), ew.get("name"), ew.get("groupId"), ew.get("lastModifiedTime")] for ew in ews]

        cloudlets = cloudlets_list_policies(api)
        products_rows["cloudlets_policies.csv"] = [[cl.get("policyId"), cl.get("name"), cl.get("cloudletType"), cl.get("status")] for cl in cloudlets]

        cw = cloud_wrapper_list(api)
        products_rows["cloud_wrapper.csv"] = [[c.get("id") or c.get("containerId"), c.get("name"), c.get("status") or c.get("state")] for c in cw]
    ...
    for hr in host_records:
        usage_rows.append([
            hr.apex, hr.hostname, hr.property_name, hr.property_id, hr.property_version
        ])
    write_csv(os.path.join(out_dir, "usage_summary.csv"), ["apex", "hostname", "property_name", "property_id", "property_version"], usage_rows)

    write_csv(os.path.join(out_dir, "hostnames.csv"), ["hostname", "apex", "property_name", "property_id", "property_version"],
              [[hr.hostname, hr.apex, hr.property_name, hr.property_id, hr.property_version] for hr in host_records])

    write_csv(os.path.join(out_dir, "cps_certs.csv"), ["enrollment_id", "common_name", "sans", "status", "network"], cps_rows)

    write_csv(os.path.join(out_dir, "appsec_summary.csv"), ["config_id", "config_name", "latest_version", "num_policies"], appsec_rows)
    ...
    if args.include_products:
        for filename, rows in products_rows.items():
            headers = {
                ...
                "edgeworkers.csv": ["edgeworker_id", "name", "group_id", "last_modified"],
                "cloudlets_policies.csv": ["policy_id", "name", "cloudlet_type", "status"],
        ...
        for domain in gtm_domains:
            ...
            write_csv(
                ...
                [[m.get("name"), (m.get("defaultDatacenter") or {}).get("datacenterId"), len(m.get("assignments", []) or [])] for m in cidr_maps],
            )
            write_csv(
                os.path.join(base_dir, "as_maps.csv"),
                ["name", "defaultDatacenter", "assignmentsCount"],
                [[m.get("name"), (m.get("defaultDatacenter") or {}).get("datacenterId"), len(m.get("assignments", []) or [])] for m in as_maps],
            )
            write_csv(
                os.path.join(base_dir, "geo_maps.csv"),
                ["name", "defaultDatacenter", "assignmentsCount"],
                [[m.get("name"), (m.get("defaultDatacenter") or {}).get("datacenterId"), len(m.get("assignments", []) or [])] for m in geo_maps],
            )
